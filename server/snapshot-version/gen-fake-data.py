...
class fake_location_gen:
    ...
    def day(self, present):
        ...
        if day_of_week==0 or day_of_week==2 or (day_of_week==4 and friday_has_class):
            ...
            home_to_other = ( "home", "other", home_to_other)
            other_to_work = ( "other", "work", other_to_work)
            work_to_other = ( "work", "other", work_to_other)
            other_to_class = ( "other", "class", other_to_class)
            class_to_other = ( "class", "other", class_to_other)
            other_to_work_from_class = ( "other", "work", other_to_work_from_class)
            work_to_other_home = ( "work", "other", work_to_other_home)
            other_to_home = ( "other", "home", other_to_home )
            
            transition_list = [ home_to_other, other_to_work, work_to_other, 
                                other_to_class, class_to_other, 
                                other_to_work_from_class, work_to_other_home, 
                                other_to_home ]
            ...
            transition_list = [ home_to_other, other_to_work, 
                                work_to_other_home, other_to_home ]
            ...
            transition_list = [ home_to_other, other_to_home ]
    ...
    def generate(self):
        ...
        delta = self.timestep_min
        current_dt = self.start_dt
        while(current_dt < self.end_dt):
            # do stuff
            transitionList = self.day(current_dt)
            
            currentState = self.search(current_dt, transitionList)

            #print "%s: %s" % ( current_dt, currentState )
            s.snapshot(uid, currentState[1], current_dt)
