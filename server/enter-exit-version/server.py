...
class DatabaseHandler:
    ...
    def enter_location(self, uid, location_name, timestamp):
        converted_time = datetime.datetime.strptime(str(timestamp), "%Y%m%dT%H:%M:%S")
        return self.__database.enter_location(uid, location_name, converted_time)

    def exit_location(self, uid, location_name, timestamp):
        converted_time = datetime.datetime.strptime(str(timestamp), "%Y%m%dT%H:%M:%S")
        return self.__database.exit_location(uid, location_name, converted_time)
    ...
    def eval_model(self, uid, span_in_weeks, 
        ...
        model = self.__database.make_model(uid, span_in_weeks,15)
        print "after calling make_model"
